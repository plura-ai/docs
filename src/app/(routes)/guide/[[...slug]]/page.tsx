...
async function getGuideFromParams({ params }: { params: Promise<DocPageProps> }) {
  const parameters = await params;
  const slug = parameters.slug?.join("/") || "";
  const data = guide.find((doc) => doc.slugAsParams === slug);
...
export async function generateStaticParams(): Promise<
  { slug: string[] }[]
> {
  return guide.map((doc) => ({
    slug: doc.slugAsParams.split("/"),
  }));
}

export default async function GuidePage({ params }: { params: Promise<DocPageProps> }) {
  const doc = await getGuideFromParams({ params });
